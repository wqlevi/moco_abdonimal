...
TODO:
    ...
    - [x] multi-device training implementation
...
def create_training_options():
    ...
    parser.add_argument("--name", type=str, default="test_unet_jax")
    ...
    opt.ckp_dir = opt.ckp_dir / opt.name / get_date_time()
...
def train_step(state, rng, batch):
    ...
    rng, out, updates = ret
    state = state.apply_gradients(grads=grads) # update params 
...
def val_epoch(state, rng, batch, step, writer):
    ...
    writer.add_image(step, 'val/pred', make_grid(Tensor(np.array(out)), 4)) # [C, H*n, W]
    writer.add_image(step, 'val/gt', make_grid(Tensor(np.array(batch['gt'])), 4))
    writer.add_image(step, 'val/noisy', make_grid(Tensor(np.array(batch['noisy'])), 4))
